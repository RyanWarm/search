...
void Session::handle_query(const boost::system::error_code& error,size_t bytes_transferred) 
{
    ...
    bool cache_hit = m_server->cache_get(m_server->m_index_dir+string(m_buffer,m_recv_len),m_response);
    if(false == cache_hit){
        TB_INFO("start enquire,offset="<<offset<<",count="<<count);
        //enquire
        ...
        m_enquire->set_cutoff(1);
        Xapian::MSet matches; 
        while(1){
            try{
                matches = m_enquire->get_mset(offset,count,100000);
                if(match_set_process(matches,m_response) == false){
                    delete m_enquire;
                    return handle_close(); 
                }
                break;
        ...
        m_server->cache_set(m_server->m_index_dir+string(m_buffer,m_recv_len),m_response.data(),m_response.size());
    } 
    ...
    TB_INFO("session "<<session_id<<" search timing: "<<tv_cost.tv_sec*1000000+tv_cost.tv_usec<<" usec");
